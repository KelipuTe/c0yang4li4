...
typedef struct reactor {
  ...
  struct connection *clients;       //client连接区，正在处理的连接
  struct connection *clientsBuffer; //client连接缓冲区，未处理的连接
  int epfd;                         //监听client的socket的epoll句柄
} reactor;

// 连接线程添加client连接到client连接缓冲区
extern void connection_add(reactor *cell, connection conn);

// 连接线程的处理函数
extern void *conn_thread(void *arg);

// 通过client的socket句柄反向找到是client连接区中的哪个client连接
extern connection *connection_find(reactor *cell, int connfd);

// 连接线程删除client连接区中的client连接
extern void connection_del(reactor *cell, connection *conn);
